...
//TODO:
JS_EXPORT_API
json_object* dentry_get_flags (Entry* e)
{
    ...
    if (info != NULL)
    {
        ...
        g_object_unref(info);
        json_object_object_add(json, "read_only", json_object_new_double(is_read_only));
        json_object_object_add(json, "symbolic_link", json_object_new_double(is_symlink));
        json_object_object_add(json, "unreadable", json_object_new_double(is_unreadable));
    }

    return json;
}
JS_EXPORT_API
const char* dentry_get_name(Entry* e)
{
    TEST_GFILE(e, f)
...
JS_EXPORT_API
const char* dentry_get_appid(Entry* e)
{
...
JS_EXPORT_API
const char* dentry_get_uri(Entry* e)
{
    TEST_GFILE(e, f)
...
JS_EXPORT_API
const char* dentry_get_icon(Entry* e)
{
    char* ret = NULL;
    TEST_GFILE(e, f)
...
JS_EXPORT_API
const char* dentry_get_thumbnail(Entry* e)
{
...
JS_EXPORT_API
const char* dentry_get_id(Entry* e)
{
    const char* uri = dentry_get_uri(e);
    char* id = calc_id(uri);
    g_free((char*)uri);
    return id;
}
...
char* _get_svg_icon(Entry* entry)
{
    ...
    const char* icon = dentry_get_icon(entry);
...
GFile* dentry_create_templates(GFile* src, char* name_add_before)
{
    ...
    const char* basename = dentry_get_name(src);
    ...
    else
    {
        ...
        const char* uri = dentry_get_uri(child);
